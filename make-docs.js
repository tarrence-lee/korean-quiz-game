  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ['manifest.json', 'PWA 매니페스트 (앱 이름, 아이콘, 디스플레이 모드)'],
        ['sw.js', '서비스 워커 (오프라인 캐싱, cache-first 전략)'],
        ['icons/icon-192.png', 'PWA 홈 화면 아이콘 (192×192) — 흰색 "?" + 파란 그라데이션 배경'],
        ['icons/icon-512.png', 'PWA 스플래시/스토어 아이콘 (512×512) — 동일 디자인'],
        ['icons/gen-png.js', 'PNG 아이콘 생성 스크립트 (Node.js, SDF 선 기반 렌더링으로 "?" 마크 생성)'],
...
function featureSection() {
  return [
    ...
    bullet('진행률 프로그레스바'),
    bullet('문제당 10초 제한 시간 타이머: 원형 링 카운트다운 (10→0초)'),
    bullet('타이머 색상 변화: 초록(7초 이상) → 노랑(4~6초) → 빨강(3초 이하)'),
    bullet('타이머 일시정지: 원형 타이머를 버튼으로 구현, 클릭하면 카운트다운 정지(⏸ 표시·회색) → 다시 클릭하면 남은 시간부터 재개'),
    bullet('시간 초과 시 오답 처리: 정답 하이라이트 + "⏰ 시간 초과!" 피드백 표시'),
    bullet('Fisher-Yates 셔플로 매 세션 무작위 문제·보기 순서'),
    bullet('정답 선택 → 즉시 정답/오답 색상 표시 + 출처 안내'),
    bullet('정답 시: 0.5초 후 자동으로 다음 문제 이동'),
    bullet('오답/시간 초과 시: "다음 문제 →" 버튼으로 수동 진행'),
  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ['--clr (동적)', '#22C55E → #EF4444', '타이머 링 색상 (JavaScript가 인라인으로 갱신)'],
        ['--pct (동적)', '100% → 0%', '타이머 링 conic-gradient 진행률'],
  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ['추가 9', '문제당 10초 제한 타이머: 원형 링 UI, 색상 경보, 시간 초과 오답 처리'],
        ['추가 10', 'PWA 아이콘 재디자인: 흰색 "?" + 파란 그라데이션 (SDF 선 기반 픽셀 렌더링)'],
        ['추가 11', '타이머 일시정지 기능: 원형 타이머를 버튼화하여 클릭 시 카운트다운 일시정지/재개 (⏸ 표시, 키보드·ARIA 접근성 지원)'],
      ],
      [1440, 7920]
    ),
  ];
